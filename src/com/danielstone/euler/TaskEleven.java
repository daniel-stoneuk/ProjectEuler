...
public class TaskEleven {
    ...
    private static int getMaxProductAtIndexInRow(String grid, int index, int rowIndex, int rowSize) {
        ...
        int rightSum = rightSum(grid, index, rowIndex, rowSize);
        ...
        if (rightDiagonalSum > result) result = rightDiagonalSum;
        if (upSum > result) result = upSum;
        return result;
    }
    ...
    private static int rightDiagonalSum(String grid, int index, int rowIndex, int rowSize) {
        int product = getNumberAtIndexInRow(grid, index, rowIndex, rowSize);
        for (int i = 1; i < 4; i++) {
            if (index + i >= rowSize || rowIndex - i <= 0) return -1;
            product = product * getNumberAtIndexInRow(grid, index + i, rowIndex - i, rowSize);
        }
        return product;
    }

    private static int leftSum(String grid, int index, int rowIndex, int rowSize) {
        int product = getNumberAtIndexInRow(grid, index, rowIndex, rowSize);
        for (int i = 1; i < 4; i++) {
            if (index - i <= 0) return -1;
            product = product * getNumberAtIndexInRow(grid, index - i, rowIndex, rowSize);
        }
        return product;
    }

    private static int rightSum(String grid, int index, int rowIndex, int rowSize) {
        int product = getNumberAtIndexInRow(grid, index, rowIndex, rowSize);
        for (int i = 1; i < 4; i++) {
            if (index + i >= rowSize) return -1;
            product = product * getNumberAtIndexInRow(grid, index + i, rowIndex, rowSize);
        }
        return product;
    }

    private static int upSum(String grid, int index, int rowIndex, int rowSize) {
        int product = getNumberAtIndexInRow(grid, index, rowIndex, rowSize);
        for (int i = 1; i < 4; i++) {
            if (rowIndex - i <= 0) return -1;
            product = product * getNumberAtIndexInRow(grid, index, rowIndex - i, rowSize);
        }
        return product;
    }
}
